...
export default function App() {
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      // Always fall back to player with a default theme so music still plays
      setSongData({
        theme: {
          mood: "Default",
          primaryColor: "#FF3366",
  ...
  if (appState === 'player' && songData && audioUrl) {
    return (
      <PlayerView
        metadata={metadata ?? { title: 'Unknown', artist: 'Unknown', coverUrl: null }}
        audioUrl={audioUrl}
